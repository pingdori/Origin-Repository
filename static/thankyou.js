...
let userUrl = "http://54.243.128.73:3000/api/user";
...
async function userCheck() {
	let loginClick = document.querySelector(".loginClick");
	let logoutClick = document.querySelector(".logoutClick");
	let fetchApi = await fetch(userUrl);
...
async function logoutClick() {
	...
	let loginClick = document.querySelector(".loginClick");
	let logoutClick = document.querySelector(".logoutClick");
	let fetchApi = await fetch(userUrl, request);
	let jsonData = await fetchApi.json();
	if (jsonData.ok) {
		loginClick.setAttribute("id", "loginClick");
		logoutClick.setAttribute("id", "hide1");
...
let getUrlString = location.search;
let urlSplit = getUrlString.split("=");
let orderNumber = document.querySelector("#orderNumber")
orderNumber.textContent = urlSplit[1]