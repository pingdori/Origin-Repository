...
//傳入首頁瀑布流
const endpoint = "http://192.168.1.137:3000/api/attractions";
index(endpoint, 0);
//尋找資料>給定初次搜尋某關鍵字的頁數
...
async function getError(pages, searchValue) {
	let keywordValue = document.querySelector("#searchData").value;
	let fetchApi = await fetch(`http://192.168.1.137:3000/api/attractions?page=${pages}&keyword=${keywordValue}`);
...
function dataPrint(jsonData) {
	...
	for (i = 0; i < jsonData.data.length; i++) {
		...
		post.innerHTML = `
        <div id="img"><img src="${images}"></div>
        <p id="nametext"><a href="http://192.168.1.137:3000/api/attractions/${id}">${names}<a></p>
        <div class="texts">
        <p id="mrttext">${mrts}</p>
        <div class="textContainer"></div>
        <p id="text">${categories}</p>`;
...
function infiniteScroll(pages) {
	...
	const callback = ([entry]) => {
		if (entry.isIntersecting) {
			let ready = false;
			if (pages) {
				index("http://192.168.1.137:3000/api/attractions", pages);
				pages = false;
